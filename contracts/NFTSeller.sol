...
contract NFTSeller {
    ...
    /**
     * @dev
     * Evento que certifica que se ha cerrado satisfactoriamente la Preventa de NFTs
     */
    event ClosedSale(uint256 tokenId);

    /**
     * @dev
     * Evento que certifica que se ha cerrado fallidamente la Preventa de NFTs
     */
    event FailedSale(string name_);

    /**
     * @dev
     * Evento que informa sobre la apertura de una nueva preventa de NFTs
     */
    ...
    /**
     * @dev
     * Evento que informa de la venta en ether de un NFT de preventa
     */
    event ETHsold(string name_, uint256 amount_, address buyer_);

    /**
     * @dev
     * Evento que informa de la venta de un NFT mediante otro token
     */

    event TokenExchanged(string name_, uint256 cant_, address paytoken_, address buyer_);

    // @dev datos de las preventas
    mapping(string => Preventa) private _preventas; // implica que sólo puede venderse una vez

    // @dev mapa compras individuales. La estructura es: (NFT -> comprador -> Array de sus compras)
    mapping(string => mapping(address => Compra[])) private _compras;

    // @dev mapa precios de cada token. La estructura es: (NFT de la preventa -> token con el que está permitido pagar -> precio)
    mapping(string => mapping(address => uint256)) private _precios;
    ...
    mapping(string => mapping(address => uint256)) private _cantidades;
    ...
    function isTokenCreated(string memory token_) private view returns (bool) {
    ...
    function areCapsValid(uint256 hardcap_, uint256 softcap_) private pure returns (bool) {
        return (hardcap_ != 0 && softcap_ <= hardcap_ && hardcap_ / 2 <= softcap_); //La última es respecto a lo que hemos visto sobre que normalmente se pide que softcap tenga más de la mitad de hardcap
    }
    ...
    function getSaleInfo(string memory token) public view returns (Preventa memory preventa) {
        require(isTokenCreated(token), "NFT not available");
    ...
    function initSale(
        string memory name_,
        ERC721 NftObject_,
        uint256[] memory NFTsIds_,
        uint256 softCap_,
        uint256 endDate_,
        uint256 maxPerPerson_,
        ...
        uint256 hardCap_ = NFTsIds_.length;
        require(areCapsValid(hardCap_, softCap_), "Caps not valid");
        require(precios_.length == tokensdepago_.length, "Arrays have different length");
        console.log("*>Se han pasado los require");

        _preventas[name_] = Preventa(
            msg.sender,
            NftObject_,
            hardCap_,
            NFTsIds_,
        ...
        for (uint i = 0; i < tokensdepago_.length; i++) {
            _precios[name_][tokensdepago_[i]] = precios_[i];
        }

        console.log("*>Funcion ejecutada con exito");

        emit OpenedSale(name_, hardCap_, softCap_, endDate_, precios_, tokensdepago_, returnable_);
    ...
    function setSale(
        ...
        require(_preventas[tokenaddress_].preSaleFinished == 0, "Sale not open");
        require(isTokenCreated(tokenaddress_), "Token not created");
        require(msg.sender == _preventas[tokenaddress_].owner, "Not owner of the presale");
        ...
        for (uint i = 0; i < changedPrices_.length; i++) {
            if (_precios[tokenaddress_][addressOfChangedPrices_[i]] == 0) {
                _preventas[tokenaddress_].tokensOnceAllowed.push(addressOfChangedPrices_[i]);
            }
            _precios[tokenaddress_][addressOfChangedPrices_[i]] = changedPrices_[i];
        }
        ...
        _preventas[tokenaddress_] = Preventa(
            ...
            _preventas[tokenaddress_].NftObject,
    ...
    function buyNFTsByToken(
        ...
        require(_precios[token_][payToken_] != 0, "Not allowed to pay with this token");
        require(preventa.maxPerPerson >= amount_, "More than possible for a single buyer");
        require(preventa.amountleft >= amount_, "Not enough selling token"); // No ha de comprar más de lo que queda

        uint256 cant = (_precios[token_][payToken_] * amount_) / 1000000000000000000;
        ...
        for (uint i = 0; i < cant; i++) {
            _preventas[token_].NftObject.safeTransferFrom(
                address(this),
                msg.sender,
                _preventas[token_].NFTsIds[i]
            );
            _preventas[token_].NFTsIds.pop();
        }
        ...
        console.log("*>Se registra la compra en el mapa de compras y cantidades");
        if (
            _preventas[token_].amountleft == 0 ||
            (!isDateFuture(preventa.endDate) &&
                preventa.NFTsIds.length - preventa.amountleft >= preventa.softCap)
        ) {
    ...
    function buyNFTsByETH(string memory token_) external payable {
        ...
        uint256 cantporEth = 1000000000000000000 * (msg.value / _precios[token_][address(0)]);

        require(
            _preventas[token_].maxPerPerson >= cantporEth,
            "More than possible for a single buyer"
        );
     ...
     * token_: dirección del token
     */
    function myPurchases(string memory token_) external view returns (Compra[] memory) {
        require(isTokenCreated(token_), "No token");
        require(_compras[token_][msg.sender].length > 0, "You have no purchases");
        Compra[] memory aux;
        for (uint i = 1; i < _compras[token_][msg.sender].length; i++) {
        ...
        if (amount_ == 0) {
            ...
            for (uint i = 0; i < sales.length; i++) {
                if (sales[i].amount > 0) {
                    ...
                    sales[i].amount = 0;
                    _cantidades[token_][sales[i].token] -= sales[i].amountPayToken;
                    ...
                    if (sales[i].token == address(0)) {
                        ...
                        vendedor.transfer(sales[i].amountPayToken / 1000000000000000000); //Enviamos el ETH que pagó
                    } else {
            ...
            _preventas[token_].NftObject.safeTransferFrom(msg.sender, address(this), amount_); //Pasamos sus tokens al contrato de compraventa
            ...
            uint256 amountToSend = amount_ * (compra.amountPayToken / compra.amount);
            ...
            _compras[token_][msg.sender][i] = Compra(cantidadRestante, compra.token, pagoRestante);
        }
        console.log("*>Funcion ejecutada con exito");
    }
    ...
    function closeSale(string memory token_) external {
        ...
        bool failed = false;
        if (
            _preventas[token_].NFTsIds.length - _preventas[token_].amountleft <
    ...
    function _closeSale(string memory token_, bool failed_) private {
        require(_preventas[token_].preSaleFinished == 0, "Preventa ya cerrada");
        ...
        if (failed_) {
            ...
            for (uint i = 0; i < _preventas[token_].tokensOnceAllowed.length; i++) {
                ...
                if (_cantidades[token_][payToken] != 0) {
                    ...
                    if (payToken == address(0)) {
                        address payable vendedor = payable(_preventas[token_].owner);
